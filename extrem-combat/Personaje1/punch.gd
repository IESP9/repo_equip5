...
func _Play_Attack_Animation():
	if currentAttack == 0:
		sprite.play("punch")
		state_label.text = "Punch 1"
		%P1Hitbox.set_deferred("disabled", false)
	elif currentAttack == 1:
		state_label.text = "Punch 2"
		%P1Hitbox.set_deferred("disabled", true)
...
func _Reset_Combo():
	%P1Hitbox.set_deferred("disabled", true)
	%P2Hitbox.set_deferred("disabled", true)
	isInCombo = false
	currentAttack = 0
	time = timeTillNextInput
